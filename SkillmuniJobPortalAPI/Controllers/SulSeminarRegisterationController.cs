...
// Type: m2ostnextservice.Controllers.SulSeminarRegisterationController
    ...
    public class SulSeminarRegisterationController : ApiController
  {
    public HttpResponseMessage Post([FromBody] SeminarReg Sem)
    {
      ...
      try
      {
        ...
        {
          semResponse.id_register = m2ostnextserviceDbContext.Database.SqlQuery<int>("insert into tbl_sul_seminar_user_registration(id_seminar,id_user,status,update_date_time,slot) values({0},{1},{2},{3},{4});select max(id_register) from tbl_sul_seminar_user_registration", (object) Sem.id_seminar, (object) Sem.id_user, (object) "A", (object) DateTime.Now, (object) Sem.slot).FirstOrDefault<int>();
