...
// Type: m2ostnextservice.Controllers.AuthenticateLoginController
...
namespace m2ostnextservice.Controllers
{
    ...
    public class AuthenticateLoginController : ApiController
  {
    ...
    public HttpResponseMessage Post([FromBody] B2BUser user)
    {
      ...
      tbl_user tblUser = this.db.tbl_user.SqlQuery(" select * from tbl_user where USERID='" + user.USERID + "' and PASSWORD='" + user.PASSWORD + "' and ID_USER in (select ID_USER from tbl_user_device_link where DEVICE_ID='" + user.IMEI + "')").FirstOrDefault<tbl_user>();
      if (tblUser != null)
      {
      ...
      string str = this.db.tbl_user.SqlQuery("select * from tbl_user where USERID='" + user.USERID + "' and PASSWORD='" + user.PASSWORD + "' ").FirstOrDefault<tbl_user>() == null ? "Invalid Username and Password..." : "Device not Registered with Cafe Style Culture Club...";
