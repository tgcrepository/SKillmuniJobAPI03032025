...
// Type: m2ostnextservice.Controllers.getUnreadBriefsForAcademyController
    ...
    public class getUnreadBriefsForAcademyController : ApiController
  {
    ...
    public HttpResponseMessage Get(int UID, int OID, int AcadamyTileId)
    {
      ...
      using (m2ostnextserviceDbContext m2ostnextserviceDbContext = new m2ostnextserviceDbContext())
      {
        ...
        briefRestriction2 = m2ostnextserviceDbContext.Database.SqlQuery<tbl_academy_level_brief_restriction>("select * from tbl_academy_level_brief_restriction where id_academy = {0}", (object) AcadamyTileId).FirstOrDefault<tbl_academy_level_brief_restriction>();
        tbl_brief_tile_level_brief_restriction briefRestriction3 = m2ostnextserviceDbContext.Database.SqlQuery<tbl_brief_tile_level_brief_restriction>("select * from tbl_brief_tile_level_brief_restriction where  id_academy={0}", (object) AcadamyTileId).FirstOrDefault<tbl_brief_tile_level_brief_restriction>();
        if (briefRestriction2 != null)
        {
          if (briefRestriction3.time == 1)
          {
            DateTime date = DateTime.Now.Date;
            restrictionUserLogList = m2ostnextserviceDbContext.Database.SqlQuery<tbl_restriction_user_log>("select * from tbl_restriction_user_log where UID = {0} and  date(updated_date_time)={1} and id_academy={2}", (object) UID, (object) date, (object) AcadamyTileId).ToList<tbl_restriction_user_log>();
          }
          else if (briefRestriction3.time == 2)
          {
            int hour = DateTime.Now.Hour;
            DateTime date = DateTime.Now.Date;
            restrictionUserLogList = m2ostnextserviceDbContext.Database.SqlQuery<tbl_restriction_user_log>("select * from tbl_restriction_user_log where UID = {0}  and EXTRACT(HOUR  FROM updated_date_time)={1} and  date(updated_date_time)={2} and id_academy={3}", (object) UID, (object) hour, (object) date, (object) AcadamyTileId).ToList<tbl_restriction_user_log>();
          }
        }
      }
      ...
      foreach (tbl_brief_master tblBriefMaster in tblBriefMasterList)
      {
        ...
        using (m2ostnextserviceDbContext m2ostnextserviceDbContext = new m2ostnextserviceDbContext())
        {
          ...
          tbl_brief_user_feedback_master userFeedbackMaster = m2ostnextserviceDbContext.Database.SqlQuery<tbl_brief_user_feedback_master>("select * from tbl_brief_user_feedback_master where UID={0} and  id_brief_master= {1} and updated_date_time= (SELECT MAX(updated_date_time) FROM tbl_brief_user_feedback_master WHERE UID={2} and  id_brief_master ={3} );", (object) UID, (object) itm.id_brief_master, (object) UID, (object) itm.id_brief_master).FirstOrDefault<tbl_brief_user_feedback_master>();
          if (userFeedbackMaster != null)
          {
            briefApiResource.liked = userFeedbackMaster.liked;
            briefApiResource.disliked = userFeedbackMaster.disliked;
          }
          else
          {
      ...
      if (source == null)
        return namespace2.CreateResponse<List<BriefAPIResource>>(this.Request, HttpStatusCode.NoContent, briefApiResourceList1);
      List<BriefAPIResource> list2 = source.Where<BriefAPIResource>((Func<BriefAPIResource, bool>) (t => t.read_status == 0)).ToList<BriefAPIResource>();
      if (briefRestriction2 == null)
        return namespace2.CreateResponse<List<BriefAPIResource>>(this.Request, HttpStatusCode.OK, list2);
      int count = briefRestriction2.brief_count - restrictionUserLogList.Count;
      return namespace2.CreateResponse<List<BriefAPIResource>>(this.Request, HttpStatusCode.OK, list2.Take<BriefAPIResource>(count).ToList<BriefAPIResource>());
    }
  }
}
