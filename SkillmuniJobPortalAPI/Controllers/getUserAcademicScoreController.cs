...
// Type: m2ostnextservice.Controllers.getUserAcademicScoreController
    ...
    public class getUserAcademicScoreController : ApiController
  {
    public HttpResponseMessage Get(int UID, int OID, int AcademicTileId)
    {
      ...
      using (m2ostnextserviceDbContext m2ostnextserviceDbContext = new m2ostnextserviceDbContext())
      {
        ...
        List<tbl_user_game_score_log> list1 = m2ostnextserviceDbContext.Database.SqlQuery<tbl_user_game_score_log>("select * from tbl_user_game_score_log where id_game={0} and id_user={1} and id_academic_tile={2} ", (object) id_game, (object) UID, (object) AcademicTileId).ToList<tbl_user_game_score_log>();
        double num1 = 0.0;
        ...
        List<tbl_user_game_special_metric_score_log> list2 = m2ostnextserviceDbContext.Database.SqlQuery<tbl_user_game_special_metric_score_log>("select * from tbl_user_game_special_metric_score_log where id_game={0} and id_user={1} and id_academic_tile={2} ", (object) id_game, (object) UID, (object) AcademicTileId).ToList<tbl_user_game_special_metric_score_log>();
        double num2 = 0.0;
        ...
        List<UserBadge> badgeList = new UniversityScoringlogic().getBadgeList(UID, id_game);
        foreach (tbl_badge_master tblBadgeMaster in list3)
        {
          ...
          foreach (UserBadge userBadge in badgeList)
          {
            if (userBadge.id_badge == tblBadgeMaster.id_badge)
            {
              ...
              tblBadgeMaster.eligiblescore = Convert.ToInt32(userScoreResponse.userscore / (double) userBadge.eligible_score);
            }
          }
