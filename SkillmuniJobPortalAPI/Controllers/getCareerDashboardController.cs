...
// Type: m2ostnextservice.Controllers.getCareerDashboardController
...
namespace m2ostnextservice.Controllers
{
    ...
    public class getCareerDashboardController : ApiController
  {
    ...
    private CompanyJobPoint getCompanyJobPoint(int CID, CompanyDetail RID, int OID, int UID)
    {
      ...
      JobPoint jobPoint4 = this.db.Database.SqlQuery<JobPoint>("SELECT SUM(no_of_question * job_points_for_ra) job_point, 0 attempt_no FROM tbl_ce_career_evaluation_master WHERE id_organization =  " + OID.ToString() + " AND ce_assessment_type = 1 AND status = 'A'").FirstOrDefault<JobPoint>();
      if (jobPoint4 != null)
        num4 = jobPoint4.job_point;
